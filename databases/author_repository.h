...
private:
    SQLite::Database db_;
    void printTable(const std::vector<Author>& authors);

public:
    AuthorRepository(const std::string& db_path = "library.db");
    bool initialize();
    bool authorExists(const Author& author);
    int save(const Author& author);
    void showAll();
